...
import { UploadCloud, Database, Settings, TableProperties, Download, Play, Plus, Trash2, CheckCircle2, FileSpreadsheet, Save, FolderOpen, Globe, TerminalSquare, X, Maximize2, Minimize2, RotateCcw, Search, ChevronUp, ChevronDown, Replace as ReplaceIcon, ReplaceAll } from 'lucide-react';
...
const uiText = {
  en: {
    ...
    s1Box3: "Recipe Dictionary (Optional)", s1Click3: "Click to select recipes.csv",
    s2Title: "2. Map Your Fields", s2Desc: "Tell WienerCalc which columns represent IDs and amounts in your CSVs.",
    s2FoodId: "Food ID Column Name", s2InputId: "Input ID Column Name", s2Select: "Select a column...",
    s2Amount: "Amount Column Name", s2Scale: "Input Scale (Multiplier)", s2ScaleHint: "E.g., 0.01 converts grams to 100g portions.",
    ...
    s2AliasTitle: "Column Aliases (Merge Recipes & Foods)", s2AliasDesc: "Link columns from your recipe table to your food table so they add up correctly.",
    s2AddAlias: "Add Alias", s2RecipeCol: "Recipe Col.", s2FoodCol: "Food Col.", s2NoAlias: "No aliases set. Columns with the exact same name will merge automatically.",
  ...
  es: {
    ...
    s2Amount: "Nombre de Columna de Cantidad", s2Scale: "Escala de Entrada (Multiplicador)", s2ScaleHint: "Ej., 0.01 convierte gramos a porciones de 100g.",
    ...
    s2AliasTitle: "Alias de Columnas (Unir Recetas y Alimentos)", s2AliasDesc: "Empareja las columnas de tu tabla de recetas con la tabla principal para que se sumen bien.",
    s2AddAlias: "Añadir Alias", s2RecipeCol: "Col. Recetas", s2FoodCol: "Col. Alimentos", s2NoAlias: "No hay alias. Las columnas que se llamen exactamente igual se fusionarán automáticamente.",
...
interface ColumnAlias { recipeCol: string; foodCol: string; }
interface WienerConfig { foodsFilePath: string; inputFilePath: string; recipesFilePath?: string; foodIdCol: string; inputIdCol: string; amountCol: string; inputScale: number; cookMethodCol: string; calculations: CalculationRule[]; cookRules: CookRule[]; groupByCol: string; columnAliases: ColumnAlias[]; }
...
export default function App() {
  ...
  const [recipeHeaders, setRecipeHeaders] = useState<string[]>([]);
  ...
  const [config, setConfig] = useState<WienerConfig>({
    foodsFilePath: '', inputFilePath: '', recipesFilePath: '', foodIdCol: 'codalim', inputIdCol: 'codalim', amountCol: 'cantiprep',
    inputScale: 0.01, cookMethodCol: '', groupByCol: '', columnAliases: [],
    calculations: [], cookRules: []
  });

  const refreshFileHeaders = async (filePath: string, type: 'foods' | 'input' | 'recipes') => {
    try {
      const headers = await window.wienerApi.getCsvHeaders(filePath);
      if (type === 'foods') setFoodHeaders(headers);
      else if (type === 'recipes') setRecipeHeaders(headers);
      else {
         setInputHeaders(headers);
         if (config.cookMethodCol) {
  ...
  const handleSelectFile = async (key: 'foodsFilePath' | 'inputFilePath' | 'recipesFilePath') => {
    const filePath = await window.wienerApi.selectFile();
    if (filePath) {
      setConfig({ ...config, [key]: filePath });
      if (key === 'foodsFilePath') refreshFileHeaders(filePath, 'foods');
      if (key === 'inputFilePath') refreshFileHeaders(filePath, 'input');
      if (key === 'recipesFilePath') refreshFileHeaders(filePath, 'recipes');
    }
  };

  const handleRunCalculation = async () => {
    if (!config.foodsFilePath || !config.inputFilePath) { alert(lang === 'en' ? 'Woof! Please select both a Foods file and an Input file first.' : '¡Guau! Por favor selecciona ambos archivos.'); return; }
  ...
  const handleExportCsv = async () => {
    ...
    try { const response = await window.wienerApi.saveCsv(results); if (response.success) alert(`Exportado exitosamente a:\n${response.filePath}`); } catch (err: any) { alert(`System error: ${err.message}`); }
  };

  const handleExportExcel = async () => {
    if (!results || results.length === 0) return;
    try { const response = await window.wienerApi.saveExcel(results); if (response.success) alert(`Excel exportado exitosamente a:\n${response.filePath}`); } catch (err: any) { alert(`System error: ${err.message}`); }
  };

  const handleSaveProfile = async () => {
    try { const response = await window.wienerApi.saveConfig(config); if (response.success) alert(`Perfil guardado exitosamente en:\n${response.filePath}`); } catch (err: any) { alert(`System error: ${err.message}`); }
  };
  ...
  const handleLoadProfile = async () => {
    try {
      ...
      if (response.success && response.data) {
        setConfig(response.data); alert('¡Perfil cargado perfectamente!');
        ...
        if(response.data.recipesFilePath) refreshFileHeaders(response.data.recipesFilePath, 'recipes');
      }
    } catch (err: any) { alert(`System error: ${err.message}`); }
  };
  ...
  const handleIdeSaveComplete = (savedFileType: 'foods' | 'input' | 'recipes') => {
      const path = savedFileType === 'foods' ? config.foodsFilePath : savedFileType === 'input' ? config.inputFilePath : config.recipesFilePath;
      if(path && savedFileType !== 'recipes') refreshFileHeaders(path, savedFileType);
  }
    ...
    <div className="h-screen w-screen flex overflow-hidden bg-umbrella-deep font-sans relative">
      <div className="w-64 glass-sidebar flex flex-col z-20 shrink-0">
        <div className="p-5 border-b border-umbrella-mid/80 flex items-center space-x-3">
          <div className="w-14 h-14 shrink-0 rounded-full overflow-hidden cursor-pointer shadow-[0_0_15px_rgba(157,78,221,0.4)] hover:shadow-[0_0_25px_rgba(157,78,221,0.8)] transition-all border-2 border-umbrella-accent/40 bg-black flex items-center justify-center" onClick={() => setIsPlayingLogo(true)} title="Inicializar secuencia visual">
            {isPlayingLogo ? <video src={logoAnim} autoPlay muted onEnded={() => setIsPlayingLogo(false)} className="w-full h-full object-cover" /> : <img src={logoImg} alt="WienerCalc Logo" className="w-full h-full object-cover" />}
          </div>
          <a href="https://wienerhoundstudios.netlify.app/" target="_blank" rel="noopener noreferrer" className="text-xl font-extrabold text-umbrella-bright tracking-widest uppercase hover:text-white hover:scale-105 transition-all">WienerCalc</a>
        </div>
        ...
        <div className="p-4 border-t border-umbrella-mid/80">
          ...
          <div className="space-y-2 mb-4">
            <button onClick={handleSaveProfile} className="w-full flex items-center space-x-3 px-4 py-2.5 rounded-lg transition-all font-medium text-left text-sm text-neutral-400 hover:bg-umbrella-dark hover:text-umbrella-bright"><Save size={16} className="text-umbrella-accent" /><span>{t.saveCfg}</span></button>
            <button onClick={handleLoadProfile} className="w-full flex items-center space-x-3 px-4 py-2.5 rounded-lg transition-all font-medium text-left text-sm text-neutral-400 hover:bg-umbrella-dark hover:text-umbrella-bright"><FolderOpen size={16} className="text-umbrella-accent" /><span>{t.loadCfg}</span></button>
          </div>
          <button onClick={() => setLang(lang === 'en' ? 'es' : 'en')} className="w-full flex items-center justify-center space-x-2 px-4 py-2 glass-inner hover:bg-umbrella-mid/60 hover:text-umbrella-bright rounded-lg text-xs font-bold text-neutral-300 transition-all border-umbrella-mid/50"><Globe size={14} className="text-umbrella-bright" /><span>{t.lang}</span></button>
          <div className="mt-6 text-center"><a href="https://github.com/Gandrenix" target="_blank" rel="noopener noreferrer" className="text-[10px] font-mono text-neutral-500 hover:text-umbrella-accent transition-colors block">© Created by WienerHoundStudios</a></div>
        </div>
      </div>
      <div className="flex-1 p-8 overflow-y-auto relative">
        ...
        <div className="max-w-3xl mx-auto mt-8">
          ...
          {activeTab === 'mapping' && <StepMapping config={config} updateConfig={updateConfig} setConfig={setConfig} foodHeaders={foodHeaders} inputHeaders={inputHeaders} recipeHeaders={recipeHeaders} setUniqueMethods={setUniqueMethods} t={t} />}
  ...
  return (
    <div className="max-w-4xl animate-step-reveal">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <FileDropzone title={t.s1Box3} description={config.recipesFilePath || t.s1Click3} isActive={!!config.recipesFilePath} onClick={() => onSelectFile('recipesFilePath')} />
      </div>
    </div>
  );
}

function StepMapping({ config, updateConfig, setConfig, foodHeaders, inputHeaders, recipeHeaders, setUniqueMethods, t }: any) {
  return (
    <div className="max-w-4xl animate-step-reveal pb-10">
      ...
      <div className="glass-panel p-6 space-y-6 relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-6 mt-2">
          {['foodIdCol', 'inputIdCol', 'amountCol', 'cookMethodCol', 'groupByCol'].map((colKey, i) => (
            <div key={colKey}>
              <label className="block text-sm font-semibold mb-2 text-neutral-300 uppercase tracking-wide">
                {colKey === 'foodIdCol' ? t.s2FoodId : colKey === 'inputIdCol' ? t.s2InputId : colKey === 'amountCol' ? t.s2Amount : colKey === 'cookMethodCol' ? t.s2CookCol : t.s2Group}
              </label>
              <select value={config[colKey] || ''} onChange={async (e) => {
                  const val = e.target.value; updateConfig(colKey as keyof WienerConfig, val);
                  if (colKey === 'cookMethodCol') {
                    if (val && config.inputFilePath) setUniqueMethods(await window.wienerApi.scanUniqueValues(config.inputFilePath, val)); else setUniqueMethods([]);
                  }
          ...
          <div>
            ...
            <input type="number" value={config.inputScale} onChange={e => updateConfig('inputScale', parseFloat(e.target.value))} className="w-full p-2.5 border border-umbrella-mid rounded bg-umbrella-deep font-mono text-sm text-neutral-200 focus:outline-none focus:border-umbrella-bright focus:ring-1 focus:ring-umbrella-bright transition-all" />
            <p className="text-xs text-neutral-500 mt-2 font-mono">{t.s2ScaleHint}</p>
          </div>
        </div>
        {config.recipesFilePath && recipeHeaders.length > 0 && (
          <div className="mt-8 pt-6 border-t border-umbrella-mid/50">
            <div className="flex justify-between items-center mb-4">
              <div><h3 className="font-bold text-lg text-white uppercase tracking-wide">{t.s2AliasTitle}</h3><p className="text-neutral-400 font-mono text-sm">{t.s2AliasDesc}</p></div>
              <button onClick={() => setConfig({...config, columnAliases: [...config.columnAliases, {recipeCol: '', foodCol: ''}]})} className="flex items-center text-sm text-umbrella-bright font-bold hover:bg-umbrella-light/40 px-3 py-1.5 rounded transition">
            ...
            <div className="space-y-3">
              {config.columnAliases.map((alias: any, idx: number) => (
                <div key={idx} className="flex items-center space-x-4 glass-inner p-3">
                  <div className="w-5/12"><span className="text-xs font-bold uppercase text-neutral-400 tracking-wider block mb-1">{t.s2RecipeCol}</span>
                    <select value={alias.recipeCol} onChange={(e) => { const newAliases = [...config.columnAliases]; newAliases[idx].recipeCol = e.target.value; setConfig({...config, columnAliases: newAliases}); }} className="w-full p-2 border border-umbrella-mid rounded bg-umbrella-deep font-mono text-sm text-neutral-200 focus:outline-none focus:border-umbrella-bright">
                      <option value="">{t.s2Select}</option>{recipeHeaders.map((h: string) => <option key={h} value={h}>{h}</option>)}
                    </select>
                  </div>
                  <div className="w-2 flex justify-center text-umbrella-accent font-bold mt-5">=</div>
                  <div className="w-5/12"><span className="text-xs font-bold uppercase text-neutral-400 tracking-wider block mb-1">{t.s2FoodCol}</span>
                    <select value={alias.foodCol} onChange={(e) => { const newAliases = [...config.columnAliases]; newAliases[idx].foodCol = e.target.value; setConfig({...config, columnAliases: newAliases}); }} className="w-full p-2 border border-umbrella-mid rounded bg-umbrella-deep font-mono text-sm text-neutral-200 focus:outline-none focus:border-umbrella-bright">
                      <option value="">{t.s2Select}</option>{foodHeaders.map((h: string) => <option key={h} value={h}>{h}</option>)}
                    </select>
                  </div>
                  <div className="w-2/12 pt-5 text-right"><button onClick={() => { const newAliases = config.columnAliases.filter((_: any, i: number) => i !== idx); setConfig({...config, columnAliases: newAliases}); }} className="text-neutral-500 hover:text-red-400 p-2 transition"><Trash2 size={18} /></button></div>
                </div>
              ))}
              {config.columnAliases.length === 0 && (<div className="text-center p-4 border border-dashed border-umbrella-mid rounded-lg text-neutral-500 font-mono text-sm">{t.s2NoAlias}</div>)}
            </div>
          </div>
        )}
      </div>
    ...
    <div className="max-w-4xl animate-step-reveal">
      ...
      <div className="glass-panel p-6 mb-6 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-4 border-b border-umbrella-mid pb-3 mt-2">
          ...
          <button onClick={addCalcRule} className="flex items-center text-sm text-umbrella-bright font-bold hover:bg-umbrella-light/40 px-3 py-1.5 rounded transition"><Plus size={16} className="mr-1" /> {t.s3AddRule}</button>
        </div>
        {foodHeaders.length > 0 && (
          <div className="mb-5 p-4 bg-umbrella-deep/50 border border-umbrella-mid rounded-lg">
            ...
            <div className="flex flex-wrap gap-2">{foodHeaders.map((h: string) => <button key={h} onClick={() => navigator.clipboard.writeText(h)} className="px-2 py-1 bg-umbrella-dark border border-umbrella-mid text-umbrella-bright text-xs font-mono rounded hover:bg-umbrella-light hover:border-umbrella-bright transition">{h}</button>)}</div>
          </div>
        )}
        {config.calculations.map((calc: any, idx: number) => (
          <div key={idx} className="flex items-center space-x-3 glass-inner p-3 mb-3">
            <input type="text" value={calc.outputField} placeholder={t.s3NewField} onChange={(e) => { const newCalcs = [...config.calculations]; newCalcs[idx].outputField = e.target.value; setConfig({...config, calculations: newCalcs}); }} className="p-2 border border-umbrella-mid rounded bg-umbrella-deep w-1/4 font-mono text-sm text-neutral-200 focus:outline-none focus:border-umbrella-bright" />
            <span className="text-umbrella-accent font-bold">=</span>
            <input type="text" value={calc.expression} placeholder={t.s3Paste} onChange={(e) => { const newCalcs = [...config.calculations]; newCalcs[idx].expression = e.target.value; setConfig({...config, calculations: newCalcs}); }} className="p-2 border border-umbrella-mid rounded bg-umbrella-deep flex-1 font-mono text-sm text-umbrella-bright focus:outline-none focus:border-umbrella-bright" />
      ...
      <div className="glass-panel p-6 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-4 border-b border-umbrella-mid pb-3 mt-2">
          ...
          <button onClick={addCookRule} className="flex items-center text-sm text-umbrella-bright font-bold hover:bg-umbrella-light/40 px-3 py-1.5 rounded transition"><Plus size={16} className="mr-1" /> {t.s3AddCook}</button>
        </div>
        {config.cookRules.map((rule: any, idx: number) => (
          <div key={idx} className="flex items-start space-x-4 glass-inner p-4 mb-3">
            <div className="w-1/4"><span className="text-xs font-bold uppercase text-neutral-400 tracking-wider">{t.s3Method}</span>
              {uniqueMethods.length > 0 ? (
                <select value={rule.method} onChange={(e) => { const newRules = [...config.cookRules]; newRules[idx].method = e.target.value; setConfig({...config, cookRules: newRules}); }} className="p-2 border border-umbrella-mid rounded bg-umbrella-deep w-full font-mono text-sm mt-2 text-neutral-200 focus:outline-none focus:border-umbrella-bright">
                  <option value="">{t.s3SelMethod}</option>{uniqueMethods.map((m: string) => <option key={m} value={m}>{m}</option>)}
                </select>
              ) : (<input type="text" value={rule.method} placeholder="e.g., boil" onChange={(e) => { const newRules = [...config.cookRules]; newRules[idx].method = e.target.value; setConfig({...config, cookRules: newRules}); }} className="p-2 border border-umbrella-mid rounded bg-umbrella-deep w-full font-mono text-sm mt-2 text-neutral-200 focus:outline-none focus:border-umbrella-bright" />)}
            </div>
            <div className="w-1/4"><span className="text-xs font-bold uppercase text-neutral-400 tracking-wider">{t.s3Reduce}</span>
              <select value={rule.reduceField} onChange={(e) => { const newRules = [...config.cookRules]; newRules[idx].reduceField = e.target.value; setConfig({...config, cookRules: newRules}); }} className="p-2 border border-umbrella-mid rounded bg-umbrella-deep w-full font-mono text-sm mt-2 text-neutral-200 focus:outline-none focus:border-umbrella-bright">
                <option value="">{t.s2Select}</option>{foodHeaders.map((h: string) => <option key={h} value={h}>{h}</option>)}
              </select>
            </div>
            <div className="flex-1"><span className="text-xs font-bold uppercase text-neutral-400 tracking-wider">{t.s3Target}</span>
                ...
                {foodHeaders.length === 0 ? <span className="text-sm text-neutral-500 italic font-mono">{t.s3SelFirst}</span> : 
                  foodHeaders.map((h: string) => (
                    <label key={h} className="flex items-center space-x-2 text-sm cursor-pointer hover:bg-umbrella-dark p-1.5 rounded transition">
                      <input type="checkbox" checked={rule.targetNutrients.includes(h)} onChange={(e) => { const newRules = [...config.cookRules]; if (e.target.checked) newRules[idx].targetNutrients.push(h); else newRules[idx].targetNutrients = newRules[idx].targetNutrients.filter((n: string) => n !== h); setConfig({...config, cookRules: newRules}); }} className="rounded bg-umbrella-deep border-umbrella-mid text-umbrella-accent focus:ring-umbrella-accent focus:ring-offset-umbrella-deep" />
...
function StepCalculate({ config, isCalculating, results, onRun, onExport, onExportExcel, t }: any) {
  // 🔥 CORRECCIÓN VISUAL: Extraer TODAS las columnas únicas de las primeras 50 filas
  const allKeys = useMemo(() => {
      if (!results || results.length === 0) return [];
      const keysSet = new Set<string>();
      results.slice(0, 50).forEach((row: any) => {
    ...
    <div className="max-w-4xl animate-step-reveal pb-10">
      <h2 className="text-3xl font-extrabold mb-6 tracking-wide text-white">{t.s4Title}</h2>
       <div className="glass-panel p-10 text-center mb-6 relative overflow-hidden">
        ...
        <h3 className="text-2xl font-bold mb-3 text-white tracking-wide">{isCalculating ? t.s4Crunching : results ? t.s4Complete : t.s4Ready}</h3>
        <p className="text-neutral-400 mb-8 max-w-md mx-auto font-mono text-sm">{results ? `STATUS: SUCCESS. ${results.length} databanks processed.` : t.s4Desc}</p>
        <div className="flex justify-center space-x-4">
          ...
          {results && (
            <>
              <button onClick={onExport} className="glass-inner hover:bg-umbrella-light/50 text-neutral-200 px-6 py-3.5 rounded-lg font-bold border-umbrella-mid transition flex items-center"><Download size={18} className="mr-2 text-umbrella-bright" /> {t.s4ExportCSV}</button>
              <button onClick={onExportExcel} className="glass-inner hover:bg-umbrella-light/50 text-neutral-200 px-6 py-3.5 rounded-lg font-bold border-umbrella-mid transition flex items-center"><FileSpreadsheet size={18} className="mr-2 text-green-400" /> {t.s4ExportExcel}</button>
            </>
          )}
        </div>
      </div>
      {results && results.length > 0 && (
        <div className="glass-panel p-1 border border-umbrella-mid overflow-hidden">
          <div className="bg-umbrella-deep/80 p-3 flex justify-between items-center border-b border-umbrella-mid"><h4 className="font-bold text-umbrella-bright tracking-widest uppercase text-xs">{t.s4Preview}</h4><span className="text-xs font-mono text-neutral-500">SYS_OUT // ROWS: 5</span></div>
              ...
              <tbody className="font-mono text-neutral-300">
                {results.slice(0, 5).map((row: any, i: number) => (
                  <tr key={i} className="border-b border-umbrella-mid/50 hover:bg-umbrella-light/20 transition-colors">
                    {allKeys.map(key => <td key={key} className="px-5 py-3">{row[key] !== undefined ? String(row[key]) : ''}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
...
function FileDropzone({ title, description, isActive, onClick }: any) {
  return (
    <div onClick={onClick} className={`border-2 border-dashed rounded-xl p-8 transition-all cursor-pointer text-center group bg-umbrella-dark/20 ${isActive ? 'border-umbrella-accent bg-umbrella-accent/10 shadow-[inset_0_0_20px_rgba(157,78,221,0.1)]' : 'border-umbrella-mid hover:border-umbrella-bright/50 hover:bg-umbrella-dark/40'}`}>
      <div className={`flex justify-center mb-4 transition-colors ${isActive ? 'text-umbrella-accent' : 'text-neutral-600 group-hover:text-umbrella-bright'}`}><UploadCloud size={40} /></div>
...
// ==========================================
// IDE TERMINAL CSV OPTIMIZADO (V5.0 - Perfect Virtualization & Memory Map)
// ==========================================
...
function CsvTerminal({ config, onClose, onToggleWidth, isWide, onFileSaved }: any) {
  const [activeFile, setActiveFile] = useState<'foods' | 'input' | 'recipes'>('foods');
  ...
  const matchMap = useMemo(() => {
      if (!searchQuery || searchMode === 'none') return { positions: [], count: 0 };
      const positions: {lineIndex: number, colIndex: number, subIndex: number}[] = [];
      const queryLower = searchQuery.toLowerCase();
      const regex = new RegExp(`(${escapeRegex(searchQuery)})`, 'gi');

      for (let i = 1; i < parsedLines.length; i++) {
          if (parsedLines[i].toLowerCase().indexOf(queryLower) !== -1) {
              const cells = parsedLines[i].split(',');
              for (let j = 0; j < cells.length; j++) {
                  if (cells[j].toLowerCase().indexOf(queryLower) !== -1) {
                      const matchesInCell = cells[j].match(regex);
                      if (matchesInCell) {
                          for (let k = 0; k < matchesInCell.length; k++) {
                              positions.push({ lineIndex: i, colIndex: j, subIndex: k });
                          }
                      }
                  }
              }
  ...
  const navigateMatch = (direction: 'next' | 'prev') => {
      ...
      if (direction === 'next') nextIndex = (activeMatch + 1) % matchMap.count;
      if (direction === 'prev') nextIndex = (activeMatch - 1 + matchMap.count) % matchMap.count;
  ...
  useEffect(() => {
      if (matchMap.count > 0 && searchMode !== 'none') {
          ...
          if (target && textareaRef.current) {
              const targetY = Math.max(0, (target.lineIndex - 1) * LINE_HEIGHT - 100); 
              textareaRef.current.scrollTop = targetY;
          }
      }
  }, [activeMatch, matchMap.count, searchMode]);

  const handleRevert = () => { setContent(savedContent); setStatusMsg('SYS_SYNCED'); };
  ...
  const handleScroll = (e: React.UIEvent<HTMLTextAreaElement>) => {
    ...
    if (Math.abs(currentLine - scrollIndex) > 10) {
        setScrollIndex(currentLine);
    }
  };

  const executeReplaceCurrent = () => {
      ...
      cells[target.colIndex] = cells[target.colIndex].replace(new RegExp(escapeRegex(searchQuery), 'gi'), (match) => {
          if (subMatchCounter === target.subIndex) { subMatchCounter++; return replaceQuery; }
          subMatchCounter++; return match;
      });
      newLines[target.lineIndex] = cells.join(','); setContent(newLines.join('\n'));
      if (activeMatch >= matchMap.count - 1) setActiveMatch(Math.max(0, matchMap.count - 2));
  };

  const executeReplaceAll = () => {
      if (!searchQuery) return;
      const newContent = content.replace(new RegExp(escapeRegex(searchQuery), 'gi'), replaceQuery);
      setContent(newContent); setActiveMatch(0);
  };
  ...
  const renderVirtualHighlights = () => {
    ...
    const visibleLines = bodyLines.slice(start, end);
    ...
    <div className="w-full h-full flex flex-col relative animate-boot">
      ...
      <div className="h-12 border-b border-umbrella-mid/50 flex items-center justify-between px-4 bg-umbrella-deep/90 z-50 flex-shrink-0">
        <div className="flex space-x-2">
          ...
          <button onClick={onToggleWidth} className="w-3.5 h-3.5 rounded-full bg-yellow-500 hover:bg-yellow-400 flex items-center justify-center group transition-colors cursor-pointer">{isWide ? <Minimize2 size={10} className="text-yellow-900 opacity-0 group-hover:opacity-100"/> : <Maximize2 size={10} className="text-yellow-900 opacity-0 group-hover:opacity-100"/>}</button>
          <button onClick={handleRevert} disabled={!isModified || isSaving} className="w-3.5 h-3.5 rounded-full bg-green-500 hover:bg-green-400 flex items-center justify-center group transition-colors cursor-pointer disabled:opacity-50"><RotateCcw size={10} className="text-green-900 opacity-0 group-hover:opacity-100"/></button>
        </div>
        <span className="text-[10px] font-mono text-umbrella-accent tracking-[0.2em] font-bold">W.C.U.C.I_TERMINAL_v5.0</span>
      </div>

      <div className="flex border-b border-umbrella-mid/30 bg-[#030105] text-xs font-mono z-30 flex-shrink-0">
        <button onClick={() => setActiveFile('foods')} className={`flex-1 py-2.5 px-4 transition-all ${activeFile === 'foods' ? 'bg-[#08020d] text-umbrella-bright shadow-[inset_0_-2px_0_#9d4edd]' : 'text-neutral-600 hover:text-neutral-400'}`}>[ foods.csv ]</button>
        <button onClick={() => setActiveFile('input')} className={`flex-1 py-2.5 px-4 transition-all border-l border-umbrella-mid/30 ${activeFile === 'input' ? 'bg-[#08020d] text-umbrella-bright shadow-[inset_0_-2px_0_#9d4edd]' : 'text-neutral-600 hover:text-neutral-400'}`}>[ input.csv ]</button>
        <button onClick={() => setActiveFile('recipes')} className={`flex-1 py-2.5 px-4 transition-all border-l border-umbrella-mid/30 ${activeFile === 'recipes' ? 'bg-[#08020d] text-umbrella-bright shadow-[inset_0_-2px_0_#9d4edd]' : 'text-neutral-600 hover:text-neutral-400'}`}>[ recipes.csv ]</button>
      </div>
        ...
        {searchMode !== 'none' && (
          <div className="absolute top-4 right-6 z-50 flex flex-col bg-[#0a040f] border border-umbrella-accent/50 rounded-lg shadow-2xl overflow-hidden animate-boot">
            <div className="flex items-center p-2 border-b border-umbrella-mid/30">
               ...
               <input ref={searchInputRef} type="text" value={searchQuery} onChange={e => {setSearchQuery(e.target.value); setActiveMatch(0);}} onKeyDown={handleSearchKeyDown} placeholder="Find..." className="bg-transparent border-none text-sm font-mono text-white p-1 w-40 focus:outline-none placeholder-neutral-600" />
               <span className="text-[10px] font-mono text-neutral-500 mr-2 w-16 text-center">{matchMap.count > 0 ? `${activeMatch + 1} of ${matchMap.count}` : '0 of 0'}</span>
            ...
            {searchMode === 'replace' && (
              <div className="flex items-center p-2 bg-[#06020A]">
                 ...
                 <input ref={replaceInputRef} type="text" value={replaceQuery} onChange={e => setReplaceQuery(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') executeReplaceCurrent() }} placeholder="Replace with..." className="bg-transparent border-none text-sm font-mono text-white p-1 w-32 focus:outline-none placeholder-neutral-600" />
                 <div className="flex ml-auto space-x-1">
                    <button onClick={executeReplaceCurrent} title="Replace current" className="p-1 text-neutral-400 hover:text-white hover:bg-white/10 rounded transition"><ReplaceIcon size={14}/></button>
                    <button onClick={executeReplaceAll} title="Replace all" className="p-1 text-neutral-400 hover:text-white hover:bg-white/10 rounded transition"><ReplaceAll size={14}/></button>
                 </div>
              </div>
            )}
          </div>
        ...
        <div ref={headerScrollRef} className="overflow-hidden bg-[#06020A] border-b border-umbrella-mid/50 z-30 flex-shrink-0 pt-3 shadow-md" style={{ paddingLeft: '16px', paddingRight: '16px', paddingBottom: '6px' }}>
           <div className="whitespace-pre font-mono text-[13px] w-max" style={{ lineHeight: `${LINE_HEIGHT}px` }}>
             {parsedLines.length > 0 && parsedLines[0].split(',').map((cell, j, arr) => {
                 const columnColors = ['text-blue-400', 'text-green-400', 'text-yellow-400', 'text-purple-400', 'text-pink-400', 'text-orange-400'];
                 return (
                   <span key={`h-${j}`} className={`${columnColors[j % columnColors.length]} font-extrabold tracking-widest uppercase`}>{cell}{j < arr.length - 1 ? <span className="text-neutral-600">,</span> : ''}</span>
                 )
             })}
           </div>
        </div>
      ...
      <div className="h-12 border-t border-umbrella-mid/50 bg-[#030105] flex items-center justify-between px-4 z-30 flex-shrink-0">
        <div className="flex items-center space-x-3">
          <div className={`w-2 h-2 rounded-full shadow-[0_0_8px_currentColor] ${statusMsg === 'SYS_ERROR' ? 'bg-red-500 text-red-500 animate-pulse' : isModified ? 'bg-yellow-400 text-yellow-400 animate-pulse' : statusMsg === 'SYS_SYNCED' ? 'bg-green-500 text-green-500' : 'bg-blue-500 text-blue-500'}`}></div>
          <span className={`text-[11px] font-mono font-bold tracking-wider ${isModified ? 'text-yellow-400' : statusMsg === 'SYS_ERROR' ? 'text-red-500' : 'text-green-500'}`}>{isModified ? 'MODIFIED (UNSAVED)' : statusMsg}</span>
        </div>
        <button onClick={handleSave} disabled={!currentPath || !isModified || isSaving} className={`text-[11px] font-mono font-bold px-4 py-1.5 rounded transition-all flex items-center z-50 ${!isModified ? 'bg-neutral-900 text-neutral-600 border border-neutral-800 cursor-not-allowed' : 'bg-umbrella-accent text-white shadow-[0_0_15px_rgba(157,78,221,0.5)] hover:bg-umbrella-bright hover:scale-105'}`}>
